...
function GoogleButton() {
  ...
  const handleGoogle = async () => {
    setLoading(true);
    setError('');
    const { error: err } = await oauthService.signInWithGoogle();
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 active:scale-[.98] transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed shadow-sm"
      >
        {loading ? (
